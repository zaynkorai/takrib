...
import (
	...
	"github.com/zaynkorai/takrib/pkg/api/user/transport"
...
func TestCreate(t *testing.T) {
	cases := []struct {
		...
		},
		{
			...
			rbac: &mock.RBAC{
				AccountCreateFn: func(c echo.Context, roleID takrib.AccessRole, companyID, locationID int) error {
...

		{
			...
			udb: &mockdb.User{
				CreateFn: func(db orm.DB, usr takrib.User) (*takrib.User, error) {
					usr.ID = 1
...
func TestList(t *testing.T) {
	...
	cases := []struct {
		...
		},
		{
			...
			udb: &mockdb.User{
				ListFn: func(db orm.DB, q *takrib.ListQuery, p *takrib.Pagination) ([]takrib.User, error) {
		...
		},
		{
			...
			udb: &mockdb.User{
				ViewFn: func(db orm.DB, id int) (*takrib.User, error) {
					return &takrib.User{
						Role: &takrib.Role{
							AccessLevel: takrib.CompanyAdminRole,
						},
					}, nil
				},
			},
