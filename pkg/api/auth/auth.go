...
func (a *Auth) Authenticate(c echo.Context, user, pass string) (*takrib.AuthToken, error) {
	...
	if !u.Active {
		return nil, takrib.ErrUnauthorized
	}
	...
	return &takrib.AuthToken{Token: token, Expires: expire, RefreshToken: u.Token}, nil
}

// Refresh refreshes jwt token and puts new claims inside
func (a *Auth) Refresh(c echo.Context, token string) (*takrib.RefreshToken, error) {
	...
	return &takrib.RefreshToken{Token: token, Expires: expire}, nil
}

// Me returns info about currently logged user
func (a *Auth) Me(c echo.Context) (*takrib.User, error) {
	au := a.rbac.User(c)
